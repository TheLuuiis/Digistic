...
window.addEventListener('DOMContentLoaded', () => {
  ...
  contact.reveal('header img', { origin: 'top' });
  contact.reveal('header nav ul li', { origin: 'top', interval: 100 });
  contact.reveal('.networks-header', { origin: 'top' });

  function transitionToContact(event) {
    ...
    document.body.style.transition = 'all 3s cubic-bezier(0.645, 0.045, 0.355, 1)';
    document.body.style.transform = 'perspective(1200px) rotateY(720deg) scale(0.7)';
  ...
  const contactLink = document.querySelector('header nav ul li a[href="contact.html"]');
  if (contactLink) {
    contactLink.addEventListener('click', transitionToContact);
  }
  ...
  window.addEventListener('mousemove', function(e) {
    ...
    document.querySelectorAll('.container-community img').forEach((img, index) => {
      const depth = (index + 1) * 0.1;
      img.style.transform = `translate3d(${mouseX * 50 * depth}px, ${mouseY * 50 * depth}px, 0)`;
    });
  });
